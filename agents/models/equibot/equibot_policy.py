...
from agents.models.equibot.vision.sim3_encoder import SIM3Vec4Latent
from agents.models.equibot.utils.diffusion.ema_model import EMAModel
from agents.models.equibot.utils.equivariant_diffusion.conditional_unet1d import VecConditionalUnet1D
...
class EquiBotPolicy(nn.Module):
    def __init__(
        ...
        pc_encoder: DictConfig,
        noise_pred_net: DictConfig,
        eef_dim: int,
        dof: int = 7,
        num_eef: int = 1,
        device="cpu",
        ...
        self.num_diffusion_iters = noise_scheduler.num_train_timesteps

        self.noise_scheduler = hydra.utils.instantiate(noise_scheduler)
        self.pc_encoder = hydra.utils.instantiate(pc_encoder)
        
        self.encoder_out_dim = pc_encoder.c_dim

        self.eef_dim = eef_dim
        self.num_eef = num_eef
        self.dof = dof
        ...
        self.noise_pred_net = hydra.utils.instantiate(noise_pred_net).to(self.device)

        self.nets = nn.ModuleDict(
            {"encoder": self.pc_encoder, "noise_pred_net": self.noise_pred_net}
        )
    ...
    def _init_torch_compile(self):
        if self.use_torch_compile:
            self.encoder_handle = torch.compile(self.pc_encoder)
            self.noise_pred_net_handle = torch.compile(self.noise_pred_net)
        else:
            self.encoder_handle = self.pc_encoder
    ...
    def _convert_action_to_vec(self, ac):
    ...
    def _convert_action_to_scalar(self, eef_ac, gripper_ac=None):
    ...
    def forward(self, obs, predict_action=True, debug=False):
        ...
        if self.obs_mode == "state":
            ...
            # feat_dict = self.encoder_handle(pc, target_norm=self.pc_scale)
            center = (
        ...
        for k in self.noise_scheduler.timesteps:
            ...
            # noise_pred = self.noise_pred_net_handle(
