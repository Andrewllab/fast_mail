...
def cosine_beta_schedule(n, s=0.008, device="cpu"):
    ...
    return append_zero(
        torch.tensor(np.flip(betas_clipped).copy(), device=device, dtype=torch.float32)
    )
...
def get_iddpm_sigmas(
    n, sigma_min=0.02, sigma_max=100, M=1000, j_0=0, C_1=0.001, C_2=0.008, device="cpu"
):
    """Constructs a continuous VP noise schedule."""
    # (sigma_max ** 2) * ((sigma_min ** 2 / sigma_max ** 2) ** (step_indices / (num_steps - 1)))
    ...
    sigmas = u_filtered[
        ((len(u_filtered) - 1) / (n - 1) * step_indices).round().to(torch.int64)
    ]
...
def sample_euler(
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        eps = (
            torch.randn_like(action) * s_noise
        )  # sample current noise depnding on S_noise
...
class DPMSolver(nn.Module):
    ...
    def dpm_solver_fast(
        ...
        for i in range(len(orders)):
            ...
            if self.info_callback is not None:
                self.info_callback(
                    {"x": action, "i": i, "t": ts[i], "t_up": t, "denoised": denoised}
                )
            ...
            action = action + su * s_noise * noise_sampler(
                self.sigma(t), self.sigma(t_next)
            )
    ...
    def dpm_solver_adaptive(
        ...
        while s < t_end - 1e-5 if forward else s > t_end + 1e-5:
            ...
            error = (
                torch.linalg.norm((action_low - action_high) / delta)
                / action.numel() ** 0.5
            )
            accept = pid.propose_step(error)
            if accept:
                action_prev = action_low
                action = action_high + su * s_noise * noise_sampler(
                    self.sigma(s), self.sigma(t)
                )
                s = t
                info["n_accept"] += 1
            else:
