...
class ContinuousCamera(ContinuousDevice):
    ...
    def start_recording(self) -> bool:
        ...
        print(f"camera {self.name} start to recording")
        return True
    ...
    def get_devices(
        ...
        """
        print(
            f"Looking for {f'up to {amount}' if amount != -1 else 'all'} {type} cameras to capture {height}x{width} frames."
        )
...
class AsynchronousCamera(ContinuousCamera, Generic[T]):
    ...
    def record_single_capture(self) -> bool:
        super().record_single_capture()
