...
from environments.real_robot.hardware.hardware_cameras import ContinuousCamera
    ...
    def stop_recording(self):
        ...
        while not gopro_iface:
            if count >= 30 * 10:  # you have 30 seconds to stop the recording..
                raise Exception(
                    "Camera did not reconnect after recording. Did you forget to manually stop the recording?"
                )
            time.sleep(0.1)
            count += 1
            gopro_iface = self.get_single_interface()
