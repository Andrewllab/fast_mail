...
from environments.real_robot.hardware.hardware_devices import ContinuousDevice
...
class AudioInterface(ContinuousDevice):
    ...
    This class inherits its functions from `environments.real_robot.hardware.hardware_devices.ContinuousDevice`.
    """
    ...
    def __frame_callback(self, in_data, frame_count, time_info, _):
        ...
        if not self.timestamp:
            self.timestamp = time_info
        return (
            np.zeros(frame_count * self.audio_input.channels).tostring(),
            pyaudio.paContinue,
        )
        ...
        def get_cond_devices(amount: int = -1):
            ...
            for idx in range(0, p.get_device_count()):
                ...
                if condition(device_info):
                    ...
                    audio_ifaces.append(
                        AudioInterface(
                            f"{idx}", name=device_info["name"], audio_input=audio_input
                        )
                    )
            return audio_ifaces
...
class AsyncAudioInterface(ContinuousDevice):
    ...
    def _continuous_capture(
        ...
        while not stop_event.is_set():
            ...
            # try:
            data = stream.read(
                int(audio_input.frames_per_buffer / 8), exception_on_overflow=False
            )
    ...
    def get_specific_devices(
        ...
        """
        getter = AsyncAudioInterface.__construct_getter(
            lambda device_info: device_info["name"].startswith(iface_name)
        )
        return getter(amount)
    ...
    def get_devices(amount: int = -1, **kwargs) -> list["AsyncAudioInterface"]:
        ...
        """
        super(AsyncAudioInterface, AsyncAudioInterface).get_devices(
            amount, type="AsyncAudioInterface", **kwargs
        )
        getter = AsyncAudioInterface.__construct_getter(
            lambda device_info: int(device_info["maxInputChannels"]) > 0
            and device_info["name"] != "default"
...
class AsyncAudioInterface2(ContinuousDevice):
    ...
    def _continuous_capture(
        ...
        def callback(in_data, frame_count, time_info, status):
            ...
            # print("Appending frame..")
            next_state = (
                pyaudio.paContinue if not stop_event.is_set() else pyaudio.paAbort
            )
    ...
    def get_specific_devices(
        ...
        - `audio_iface` (list[AsyncAudioInterface]): List of found audio interfaces. If no devices are found, `[]` is returned.
        """
        getter = AsyncAudioInterface2.__construct_getter(
            lambda device_info: device_info["name"].startswith(iface_name)
        )
        return getter(amount)
    ...
    def get_devices(amount: int = -1, **kwargs) -> list["AsyncAudioInterface"]:
        ...
        - `devices` (list[AsyncAudioInterface]): List of found devices. If no devices are found, `[]` is returned.
        """
        super(AsyncAudioInterface2, AsyncAudioInterface2).get_devices(
            amount, type="AsyncAudioInterface", **kwargs
        )
        getter = AsyncAudioInterface2.__construct_getter(
            lambda device_info: int(device_info["maxInputChannels"]) > 0
