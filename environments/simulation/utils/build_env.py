...
def apply_wrappers(env, wrapper_cfgs: DictConfig = None):
    ...
    for wrapper_name in wrapper_cfgs.keys():
        match wrapper_name:
            ...
            case _:
                raise ValueError("Required wrapper is not supported: ", wrapper_name, ". Supported wrappers: [record_video, pre_process].")

    return env

def make(task_name: str, device: str, num_envs: int, render_mode: str = None, cli_args: Optional[DictConfig] = None, wrapper_cfgs: List[dict] = []):

    base_env_cfg = env_cfg_factory(task_name, device, num_envs, cli_args)

    env = gym.make(id=task_name, cfg=base_env_cfg, num_envs=num_envs, render_mode=render_mode)
