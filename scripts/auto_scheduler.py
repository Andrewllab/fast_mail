...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Poll W&B for finished training runs and launch test jobs."
    )
    parser.add_argument(
        "--project",
        type=str,
        default="PAKT_all_envs",
        help="W&B project name (default: PAKT_all_envs)",
    )
...
def save_scheduled_ids(ids: set):
    STATE_FILE.write_text(
        yaml.dump({"scheduled_run_ids": sorted(ids)}, default_flow_style=False)
    )
...
def main():
    ...
    print(f"Polling W&B project: {PROJECT}")
    ...
    while True:
        ...
        if new_ids:
            ...
            try:
                ...
                print(
                    f"Killing process after {SUBMIT_TIMEOUT}s (submitit should have scheduled by now)."
                )
                proc.kill()
                proc.wait()
